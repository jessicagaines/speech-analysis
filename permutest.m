...
% Edden M. Gerber (2025). permutest (https://www.mathworks.com/matlabcentral/fileexchange/71737-permutest), MATLAB Central File Exchange. Retrieved June 10, 2025. 
% 
...
function [t, df] = simpleTTest(x,m)
    ...
    %samplesize  = size(x,1);
    samplesize = sum(~isnan(x));
    %xmean = sum(x)/samplesize; % works faster then mean
    xmean = mean(x,"omitmissing"); % slower but handles missing values
    ...
    xstd = std(x,"omitmissing"); % slower but handles missing values
